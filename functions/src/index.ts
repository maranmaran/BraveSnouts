...
export const health = europeFunctions.https.onRequest((req, resp) => {
  console.log('Healthy');
...
export const auctionItemBidChange = europeFunctions.firestore.document("auctions/{auctionId}/items/{itemId}")
  .onUpdate(async (change, ctx) => {
    ...
    if(!after.user) {
      console.log(`User id is not present. Before:${before.user} After:${after.user}`);
      return null;
    }

    // check if new bidder
    if (after.user === before.user) {
      console.log(`Same bidder`);
      return null;
    }
...
export const compressImage = europeFunctions.storage.object().onFinalize(async (object, ctx) => {
  ...
  console.info('Image compressed and is located at ', jpegFilePath)
  return null;
});
